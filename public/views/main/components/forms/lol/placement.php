    ...
    <div class="card p-0 bg-secondary shadow-none mb-5">
        <div class="card-header p-4 w-100">
            <div class="d-flex align-items-center">
                ...
                <div class="ms-3">
                    <h4 class="mb-1">
                        <span id="last-known-rank" data-bs-toggle="tooltip" data-bs-placement="top"
                            data-bs-container="body" data-bs-title="<?= t('Please choose the rank from the last season when you had a rank.&#10;
If you only had a rank in the preseason, select that one.&#10;
Do not choose the provisional rank shown in the client now.&#10;
If you pick the wrong rank, you may need to pay the difference later.') ?>">
                            <?= t('Last Known Rank') ?>
                        </span>
                    </h4>
                    <p class="mb-0 text-muted me-2">
                        <?= t('Select the rank from the latest season, when you were ranked.') ?>
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="mb-5">
                <div class="d-flex flex-wrap gap-2 justify-content-start align-items-center">
                    ...
                    <?php for ($i = 0; $i <= 10; $i++): ?>
                        ...
                        <!-- <?= $ranks[$i] ?> -->
                        <input class="custom-checkbox" type="radio" name="start_tier" id="start_<?= $i ?>" value="<?= $i ?>"
                            <?= $i == 3 ? "checked" : null ?>>
                        <label class="btn btn-light <?= strtolower($ranks[$i]) ?>-bg" for="start_<?= $i ?>"
                            data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $ranks[$i] ?>"
                            data-bs-custom-class="<?= strtolower($ranks[$i]) ?>">
                            <img src="<?= util_rank_img('lol', 'mini', $i) ?>" class="rank-icon-mini"
                                alt="<?= $ranks[$i] ?>">
                        </label>
                    <?php endfor ?>
                </div>
                ...
                <div class="d-flex flex-wrap gap-2 mt-2 justify-content-start align-items-center" id="start_divisions">
                    ...
                    <?php for ($i = 1; $i <= 4; $i++): ?>
                        ...
                        <input class="custom-checkbox" type="radio" name="start_division" id="start_div_<?= $i ?>"
                            value="<?= $i ?>" <?= $i == 4 ? "checked" : null ?>>
                        <label class="btn btn-light" for="start_div_<?= $i ?>">
                            <span class="rank-icon-mini"><?= util_format_lol_division($i) ?></span>
                        </label>
    ...
    <div class="card p-0 bg-secondary shadow-none mb-5">
        <div class="card-header p-4 w-100">
            <div class="d-flex align-items-center">
                ...
                <h4 class="mb-0 ms-3">
                    <?= t('Matches Amount') ?>
                    <p class="mb-0 text-muted me-2">
                        <?= t('Select your desired amount of matches.') ?>
                    </p>
                </h4>
            </div>
        </div>
        <div class="card-body p-4">
            ...
            <!-- Placements Guarantee Card -->
            <div class="card mt-3 shadow-sm border-0">
                <div class="card-body d-flex align-items-center bg-light-subtle rounded">
                    <i class="fa-solid fa-shield-check fs-3 me-3"></i>
                    <div>
                        <h6 class="mb-1 fw-bold"><?= t('Placements Guarantee') ?></h6>
                        <p id="guaranteed-wins" class="mb-0 fs-sm"><?= t('No matches selected yet.') ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="card-body">
            <div class="rank-sec bg-light p-3 mb-3 mx-n4 mt-n4 border-bottom">
                <div class="d-flex align-items-center justify-content-center gap-3">
                    ...
                    <p class="mb-0">
                        <span class="fw-bold overview-count">
                            <span class="win-count">3</span> <?= t('Matches') ?>
                        </span>
                    </p>
                </div>
            </div>
...
<script>
    ...
    function updateWinrateDisplay() {
        const rank = parseInt(document.querySelector('input[name="start_tier"]:checked').value);
        const winrate = getWinRate(rank);
        const rankName = rankNames[rank];

        let rankMessage = '';
        if (rank <= 6) {
            rankMessage = `We guarantee 4/5 wins in your ${rankName} Placements. 
            If the booster loses more than 1 game you will receive 2 extra wins for each additional loss. If the booster goes one division up after placement games, your boost will be counted as completed.`;
        } else {
            rankMessage = `We guarantee 3/5 wins in your ${rankName} Placements. If the booster loses more than 2 games you will receive 2 extra wins for each additional loss. If the booster goes one division up after placement games, your boost will be counted as completed.`;
        }

        document.getElementById('guaranteed-wins').textContent = rankMessage;
    }

    const rankRadios = document.querySelectorAll('input[name="start_tier"]');
    rankRadios.forEach(radio => {
        radio.addEventListener('change', updateWinrateDisplay);
    });

    document.querySelector('input[name="start_tier"][value="3"]').checked = true; updateWinrateDisplay(); 
</script>