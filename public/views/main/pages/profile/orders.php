    ...
    <div class="row">
        ...
        <div class="col-lg-9 col-md-8 ps-lg-5 pb-5 mb-2 mb-lg-4 mt-n3 mt-md-0">
            <div class="ps-md-3 ps-lg-0 mt-md-2 py-md-4">
                <h2 class="h2 pt-xl-1">Orders List</h2>
                <div class="table-responsive border rounded">
                    <table class="table mb-0">
                        <thead>
                            <tr>
                                <th scope="col" class="bg-faded-primary">Order ID</th>
                                <th scope="col" class="bg-faded-primary">Details</th>
                                <th scope="col" class="bg-faded-primary">Status</th>
                                <th scope="col" class="bg-faded-primary">Total</th>
                                <th scope="col" class="bg-faded-primary">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($orders as $order) : ?>
                                <tr>
                                    <td>#<?= $order['id'] ?></td>
                                    <td><?= util_format_boost_overview($order['type'], $order) ?></td>
                                    <td><?= util_format_boost_status($order['status']) ?></td>
                                    <td><?= util_format_currency_display($order['currency']) . util_format_price_display($order['price']) ?></td>
                                    <td>
                                        <?php if($order['status'] == 'UNPAID'): ?>
                                        <a href="<?= BASE_URL ?>/checkout/<?= db_get_row('invoices', ['id'=>$order['invoice_id']], 1)['uuid'] ?>" class="btn btn-sm btn-light border" style="padding: 4px 15px;"><i class="fa-duotone fa-cart-shopping me-1"></i> Pay</a>
                                        <?php else: ?>
                                        <a href="<?= BASE_URL ?>/order/<?= $order['id'] ?>" class="btn btn-sm btn-light border" style="padding: 4px 15px;"><i class="fa-duotone fa-eye me-1"></i> View</a>
                                        <?php endif ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
