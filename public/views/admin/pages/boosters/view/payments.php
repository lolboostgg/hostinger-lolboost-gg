        ...
        <table class="js-datatable table table-borderless table-thead-bordered table-nowrap table-align-middle card-table" data-hs-datatables-options='{
            ...
            <tbody>
                <?php foreach ($data['payments'] as $row) : ?>
                    <tr>
                        <td class="fw-500" data-order="<?= (int)$row['id'] ?>">
                            #<?= (int)$row['id'] ?>
                        </td>
                        <td class="fw-500">
                        ...
                        <td class="fw-500 text-end <?= $row['amount'] >= 0 ? 'text-success' : 'text-danger' ?>">
                            <?= util_format_price_display($row['amount'])." ".$row['currency']  ?>
                        </td>
                        <td class="fw-500 text-end" data-order="<?= strtotime($row['created_at']) ?>"><?= util_format_date_display($row['created_at']) ?></td>
                    </tr>
                <?php endforeach; ?>
