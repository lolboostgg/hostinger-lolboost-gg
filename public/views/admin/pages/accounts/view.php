...
// esc() is idempotent, so already-escaped DB values (account titles with apostrophes
// and emojis) render as text instead of turning into "&#039;".
$h = fn($v) => esc($v);
...
$isVal  = in_array($game, ['val', 'valorant'], true);
$gameFormValue = $isVal ? 'val' : 'lol';
...
.lb-sys{
  ...
  border-radius:14px;
  /* Matched to the client order view — this was noticeably larger than every other chat. */
  padding:10px 14px;
  font-size:.86rem;
  font-weight:800;
  line-height:1.5;
    ...
    <div class="av-sidebar-card mb-3">
      ...
      <div class="av-buyer-row">
        ...
        <div style="display:flex;align-items:center;gap:6px;margin-left:auto;flex-shrink:0;">
          <button type="button" class="av-btn-primary js-admin-poke-client" data-ref-type="account" data-id="<?= $id ?>" style="font-size:.75rem;padding:5px 12px;min-height:30px;">
            <i class="fa-duotone fa-hand-point-up"></i> Poke Client
          </button>
          <a href="<?= ADMN_URL ?>/client/<?= (int)$buyer['id'] ?>" class="av-btn-ghost" style="font-size:.75rem;padding:5px 12px;flex-shrink:0;">
    ...
    <div class="av-sidebar-card mb-3">
      ...
      <div class="av-creds-list">
        ...
        foreach ($creds as $field => [$label, $icon]):
            ...
            $decodedVal = html_entity_decode((string)$val, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $safe = htmlspecialchars($decodedVal, ENT_QUOTES, 'UTF-8');
        ?>
...
<div class="offcanvas offcanvas-end custom-offcanvas item-canvas" tabindex="-1" id="editAccountCanvas" data-bs-backdrop="static" data-bs-keyboard="true" style="width:52vw!important;">
  ...
  <div class="offcanvas-body" style="flex:1!important;overflow:hidden!important;display:flex!important;flex-direction:column!important;padding:0!important;">
    <form action="<?= AJAX_URL ?>" method="POST" enctype="multipart/form-data" class="ajax-form" id="editAccountForm" style="flex:1;display:flex;flex-direction:column;overflow:hidden;min-height:0;">
      ...
      <input type="hidden" name="game"       value="<?= $h($gameFormValue) ?>">
      ...
      <div class="oc-scroll" style="flex:1;overflow-y:auto;padding:20px 22px;">
        ...
        <div class="js-edit-step" data-step="2" style="display:none;">
          <?php if ($isVal): ?>
          <div class="oc-section-label"><i class="fa-solid fa-gamepad"></i> Valorant Data</div>
          <?php
          $valRankOptions = [0=>'Unranked',1=>'Iron',2=>'Bronze',3=>'Silver',4=>'Gold',5=>'Platinum',6=>'Diamond',7=>'Ascendant',8=>'Immortal',9=>'Radiant'];
          $valPlatform = strtoupper((string)($gameData['val_platform'] ?? $server ?: ''));
          ?>
          <input type="hidden" name="current_rank" value="<?= (int)$valRank ?>">
          <input type="hidden" name="rank" value="<?= (int)$valRank ?>">
          <div class="row g-3 mb-3">
            <div class="col-md-6">
              <label class="form-label">Valorant Rank</label>
              <select class="form-select" name="val_rank" onchange="this.form.querySelector('[name=current_rank]').value=this.value;this.form.querySelector('[name=rank]').value=this.value;">
                <?php foreach ($valRankOptions as $v => $l): ?>
                  <option value="<?= $v ?>" <?= (int)$valRank === (int)$v ? 'selected' : '' ?>><?= $l ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-6">
              ...
              <select class="form-select" name="val_platform">
                <?php foreach (['EU','NA','AP','KR','BR','LATAM'] as $platform): ?>
                  <option value="<?= $platform ?>" <?= $valPlatform === $platform ? 'selected' : '' ?>><?= $platform ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-md-4">
              ...
              <input type="number" class="form-control" name="val_agent_count" value="<?= (int)$valAgentsDisplayCount ?>" min="0">
            </div>
            <div class="col-md-4">
              <label class="form-label">Weapon Skins</label>
              <input type="number" class="form-control" name="val_weapon_skins" value="<?= (int)$valWeaponSkins ?>" min="0">
            </div>
            <div class="col-md-4">
              <label class="form-label">Valorant Points</label>
              <input type="number" class="form-control" name="val_points" value="<?= (int)$valPoints ?>" min="0">
            </div>
            <div class="col-md-4">
              <label class="form-label">Radianite</label>
              <input type="number" class="form-control" name="val_radianite" value="<?= (int)$valRadianite ?>" min="0">
            </div>
            <div class="col-md-4">
              <label class="form-label">Win Rate (%)</label>
              <input type="number" class="form-control" name="val_winrate" value="<?= (int)$valWinrate ?>" min="0" max="100">
            </div>
          </div>
          <?php else: ?>
        ...
        <div class="js-edit-step" data-step="3" style="display:none;">
          ...
          <div class="row g-3 mb-3">
            ...
            $credFields = [
              ...
              'login'          => $isVal ? 'Riot Login' : 'LoL Login',
              'password'       => $isVal ? 'Riot Login Password' : 'LoL Password',
            ...
            foreach ($credFields as $field => $label):
              $fieldVal = html_entity_decode((string)($account[$field] ?? ''), ENT_QUOTES | ENT_HTML5, 'UTF-8');
            ?>
            <div class="col-md-6">
              <label class="form-label"><?= $label ?></label>
              <input type="text" class="form-control" name="<?= $field ?>" value="<?= $h($fieldVal) ?>" autocomplete="off" style="font-family:monospace;">
            </div>
            <?php endforeach; ?>
          </div>
        </div>
