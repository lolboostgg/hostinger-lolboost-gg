...
?>
<?= $this->layout('admin/layouts/main', ['meta' => ['title' => 'Edit Article #'.$data['id'].' - Admin Area | LoLBoost.gg', 'h1' => 'Edit Article #'.$data['id'], 'description' => 'Edit blog article.']]) ?>
...
/* ── Theme tokens ───────────────────────────────────────────────────────────
   ...
   primary: #5c4ae3 | danger: #ed4c78 | muted: #91989e
   ──────────────────────────────────────────────────────────────────────── */
...
.ql-toolbar { border-radius: 8px 8px 0 0; border: 1px solid #2f3235; background: #25282a; }
...
#delete-article-modal .modal-header { border-bottom: 1px solid #2f3235; }
#delete-article-modal .modal-footer { border-top: 1px solid #2f3235; }
#delete-article-modal h5 { color: #c5c8cc; }
...
<form class="form ajax-form" action="<?= AJAX_URL ?>">
    <input type="hidden" name="action"  value="admin_update_article">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="mb-0" style="font-weight:700;color:#c5c8cc;">
                <i class="fa-duotone fa-pen-to-square me-2" style="color:#5c4ae3;"></i>Edit Article
                <span style="color:#5c4ae3;">#<?= (int)$data['id'] ?></span>
            </h4>
            <div style="font-size:.82rem;color:#91989e;margin-top:.2rem;">
                Last updated: <?= util_format_date_display($data['updated_at'] ?? '') ?>
            </div>
        </div>
        <div class="d-flex gap-2">
            <a href="<?= ADMN_URL ?>/articles"
               style="display:inline-flex;align-items:center;gap:.35rem;font-size:.78rem;color:#91989e;text-decoration:none;padding:.3rem .7rem;border:1px solid #2f3235;border-radius:.45rem;background:#25282a;">
                <i class="fa-duotone fa-arrow-left"></i> Back
            </a>
            <button type="button" class="btn btn-sm"
                    style="background:rgba(237,76,120,.12);border:1px solid rgba(237,76,120,.35);color:#ed4c78;"
                    data-bs-toggle="modal" data-bs-target="#delete-article-modal">
                <i class="fa-duotone fa-trash me-1"></i> Delete
            </button>
            <button type="submit" class="btn btn-primary btn-sm">
                <span class="indicator-label"><i class="fa-duotone fa-floppy-disk me-1"></i>Update Article</span>
        ...
        <div class="card-body pt-2 pb-0">
            ...
            <div class="pkg-form-row centered">
                ...
                <input type="text" class="form-control" name="title" id="article-title"
                       value="<?= article_admin_e($data['title'] ?? '') ?>">
            </div>
            ...
            <div class="pkg-form-row">
                ...
                <textarea class="form-control" name="description" rows="3"><?= article_admin_e($data['description'] ?? '') ?></textarea>
            </div>
            ...
            <div class="pkg-form-row">
                ...
                <textarea class="form-control" name="excerpt" rows="3"><?= article_admin_e($data['excerpt'] ?? '') ?></textarea>
            </div>
            ...
            <div class="pkg-form-row">
                <label class="pkg-form-label">
                    Thumbnail
                    <small>Replace cover image</small>
                </label>
                <div>
                    <input class="form-control" type="file" name="image_url" id="image_url" accept="image/*">
                    <?php if (!empty($data['image_url'])): ?>
                    <div class="thumb-preview mt-2" id="thumb-preview-wrap">
                        <img id="thumb-preview-img" src="<?= article_admin_e($data['image_url'] ?? '') ?>" alt="Thumbnail">
                        <span id="thumb-preview-name">Current thumbnail</span>
                    </div>
                    <?php else: ?>
                    <div class="thumb-preview mt-2" id="thumb-preview-wrap" style="display:none;">
...
<form class="ajax-form" action="<?= AJAX_URL ?>">
    ...
    <input type="hidden" name="id"     value="<?= (int)$data['id'] ?>">
    <div id="delete-article-modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        <i class="fa-duotone fa-triangle-exclamation me-2" style="color:#ed4c78;"></i>Delete Article
                    </h5>
                </div>
                <div class="modal-body text-center py-4">
                    <p class="mb-1" style="font-size:.95rem;font-weight:600;color:#c5c8cc;">
                        Are you sure you want to delete this article?
                    </p>
                    <span style="font-size:.83rem;color:#91989e;">
                        "<strong><?= article_admin_e($data['title'] ?? ('Article #'.$data['id'])) ?></strong>" will be permanently removed.
                    </span>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-sm"
                            style="background:#25282a;border:1px solid #2f3235;color:#91989e;"
                            data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" class="btn btn-sm"
                            style="background:rgba(237,76,120,.15);border:1px solid rgba(237,76,120,.4);color:#ed4c78;">
                        <i class="fa-duotone fa-trash me-1"></i>Delete article
                    </button>
                </div>
            </div>
        </div>
...
(function () {
    ...
    quill.root.innerHTML = <?= json_encode(article_admin_decode_entities($data['content'] ?? '')) ?>;
