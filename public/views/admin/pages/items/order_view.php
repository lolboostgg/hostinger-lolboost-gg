...
.lb-msg__bubble,
.lb-msg--end .lb-msg__bubble { padding:.65rem .9rem; border-radius:.75rem; border:1px solid rgba(255,255,255,.1); font-size:.875rem; line-height:1.55; word-break:break-word; background:#2b2d2f; color:#d6d8dc; }
...
<div class="av-head mb-4">
  <div class="av-head-body">
    ...
    <div class="av-actions">
      ...
      <?php endif; ?>
      <?php if (!in_array($statusRaw, ['cancelled','canceled','refunded'], true)): ?>
      <form class="ajax-form d-inline" action="<?= AJAX_URL ?>" method="POST" onsubmit="return confirm('Mark this order as unsold and reverse the seller payout?')">
        <input type="hidden" name="action" value="admin_marketplace_order_unsold">
        <input type="hidden" name="kind" value="item">
        <input type="hidden" name="id" value="<?= $id ?>">
        <button type="submit" class="av-btn-danger"><i class="fa-duotone fa-rotate-left"></i> Unsold</button>
      </form>
      <?php endif; ?>
    ...
    <div class="av-sidebar-card mb-3">
      ...
      <div class="av-stat-grid">
        ...
        $statsGrid = [
            ...
            ['fa-solid fa-calendar',    'Created',     $createdAt ? date('d-m-y', strtotime($createdAt)) : '—'],
            ['fa-solid fa-rotate',      'Updated',     $updatedAt ? date('d-m-y', strtotime($updatedAt)) : '—'],
        ];
    ...
    <div class="av-sidebar-card mb-3">
      ...
      <div class="av-user-row">
        <div class="av-user-avi av-user-avi--purple">
          <?php if (!empty($seller['icon'] ?? $purchase['seller_icon'] ?? '')): ?><img src="<?= $h((string)($seller['icon'] ?? $purchase['seller_icon'])) ?>" alt=""><?php else: ?><?= strtoupper(substr($sellerName,0,1)) ?><?php endif; ?>
        </div>
        <div style="flex:1;min-width:0;">
          <?php if ($sellerId): ?>
            <a class="av-user-link" href="<?= ADMN_URL ?>/seller/<?= $sellerId ?>/profile"><?= $h($sellerName) ?></a>
          <?php else: ?>
        ...
        <?php if ($sellerId): ?>
        <a href="<?= ADMN_URL ?>/seller/<?= $sellerId ?>/profile" class="av-btn-ghost" style="font-size:.75rem;padding:5px 12px;">
      ...
      <div class="av-user-row">
        ...
        <div style="flex:1;min-width:0;">
          <?php if ($buyerId): ?>
            <a class="av-user-link" href="<?= ADMN_URL ?>/client/<?= $buyerId ?>"><?= $h($buyerName) ?></a>
          <?php else: ?>
        ...
        <?php if ($buyerId): ?>
        <a href="<?= ADMN_URL ?>/client/<?= $buyerId ?>" class="av-btn-ghost" style="font-size:.75rem;padding:5px 12px;">
          <i class="fa-solid fa-arrow-up-right-from-square"></i>
        </a>
        <?php endif; ?>
      </div>
...
(function () {
  ...
  const SELLER_AVATAR = '<?= $h((string)($seller['icon'] ?? $purchase['seller_icon'] ?? '')) ?>';
  const CLIENT_AVATAR = '<?= $h((string)($buyer['icon'] ?? $purchase['client_icon'] ?? '')) ?>';
  ...
  const INITIAL_MESSAGES = <?= json_encode(array_values((array)($chat ?? [])), JSON_UNESCAPED_SLASHES|JSON_UNESCAPED_UNICODE) ?>;
  ...
  function getFallbackAvatar(sender){ if(sender==='admin')return ADMIN_AVATAR;if(sender==='seller'&&SELLER_AVATAR)return SELLER_AVATAR;if(sender==='client'&&CLIENT_AVATAR)return CLIENT_AVATAR;return USER_AVATAR; }

  function load_message(id, m, isGrouped){
    const t = formatTime(m.time), c = decodeHtmlEntities(m.content);
    if(m.sender==='system') return `<div class="lb-syswrap"><div class="lb-sys">${c}</div><div class="lb-sys-time">${t}</div></div>`;
    const isMe=(m.sender==='admin'), isRight=(m.sender==='admin'||m.sender==='seller');
    const aCls=isRight?'lb-msg--end':'lb-msg--start', hCls=isRight?'lb-msg__head lb-msg__head--end':'lb-msg__head';
  ...
  function render_messages(raw){
    const list={};
    $.each(raw||[],(k,v)=>{if(!v||v.deleted||v.type==='deleted')return;v.sender=v.sender||v.type||'seller';v.content=v.content??v.message??v.body??'';v.time=v.time||(v.created_at?Math.floor(new Date(v.created_at.replace(' ','T')).getTime()/1000):0);list[k]=v});
    const cnt=Object.keys(list).length,sig=JSON.stringify(list);
    if(cnt>0&&sig!==chat_sig){chat_sig=sig;let html='',ls='',lid=0;$.each(list,(k,v)=>{const g=(v.sender===ls&&String(v.sender_id)===String(lid));html+=load_message(k,v,g);ls=v.sender;lid=v.sender_id});$('#chat_messages').html(html);update_scroll();msg_none=false}
    else if(!cnt&&!msg_none){$('#chat_messages').html('<div class="lb-chat-empty"><i class="fa-duotone fa-comment-dots fa-2x mb-2"></i><span class="small">No messages yet.</span></div>');msg_none=true}
  }
  function socketPayload(data){
    data=data&&data.payload?data.payload:data;
    if(!data)return;
    const orderId=String(data.order_id||'');
    if(Number(data.purchase_id||0)!==PURCHASE_ID&&orderId!==('itempurch_'+PURCHASE_ID))return;
    if(data.messages)render_messages(data.messages);
  }
  ...
  $('#lbChatForm').on('submit',function(e){
    ...
    $.ajax({url:AJAX_URL,method:'POST',data:new FormData(this),processData:false,contentType:false})
      .done((resp)=>{ document.getElementById('lbChatMessageInput').value=''; document.getElementById('lbChatImageInput').value=''; document.getElementById('lbChatImagePreviewWrap').classList.add('d-none'); if(resp&&resp.messages)render_messages(resp.messages); update_scroll(); })
  ...
  function bindSocket(socket){if(!socket||socket.__adminItemOrderBound)return;socket.__adminItemOrderBound=true;socket.on('item_chat_update',socketPayload);try{socket.emit('item_chat_init',{purchase_id:PURCHASE_ID,area:'admin'})}catch(e){}}
