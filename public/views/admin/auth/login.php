<?= $this->layout('admin/layouts/auth', ['meta' => ['title' => 'Login - Admin Area | LoLBoost.gg', 'description' => 'Overview and statistics of the website.']]) ?>

<form class="ajax-form" novalidate action="<?= AJAX_URL ?>">
    <input type="hidden" name="action" value="auth_admin_login">

    <h2 class="lb-auth-title">Sign in</h2>
    <p class="lb-auth-subtitle">Login to your admin dashboard account</p>

    <div class="lb-field">
        <label class="lb-label" for="login">Login</label>
        <input class="lb-input" type="email" name="login" id="login" tabindex="1" placeholder="login@lolboost.gg" aria-label="login@lolboost.gg" required>
