...
if (function_exists('util_get_game_by_slug')) {
    $csLolGameIcon = (string)(util_get_game_by_slug('league-of-legends')['icon'] ?? '');
}
?>
<section class="lb-shop-hero">
    <div class="lb-shop-hero__inner">
        ...
        <div>
            ...
            <h1 class="lb-shop-hero__title"><?= t('LoL Ranked Accounts') ?></h1>
            <p class="lb-shop-hero__desc"><?= t('Buy ranked premium League of Legends accounts with champions, skins, and included email access. Fast delivery, secure login details, and a minimum 14-day warranty—start playing instantly.') ?></p>
        </div>
    </div>
</section>
<style>
...
<?php
    $lbPopularBaseWhere = "sold = 0 AND active = 1 AND (game = 'lol' OR game IS NULL)";
    $lbPopularPills = [
        ['label' => 'EUW', 'attrs' => ['servers' => 'euw'], 'where' => "UPPER(server) = 'EUW'"],
        ['label' => 'EUNE', 'attrs' => ['servers' => 'eune'], 'where' => "UPPER(server) = 'EUNE'"],
        ['label' => 'NA', 'attrs' => ['servers' => 'na'], 'where' => "UPPER(server) = 'NA'"],
        ['label' => 'KR', 'attrs' => ['servers' => 'kr'], 'where' => "UPPER(server) = 'KR'"],
        ['label' => 'Diamond+', 'attrs' => ['ranks' => '7,8,9,10'], 'where' => "rank IN (7,8,9,10)"],
        ['label' => 'Master+', 'attrs' => ['ranks' => '8,9,10'], 'where' => "rank IN (8,9,10)"],
        ['label' => 'Grandmaster', 'attrs' => ['ranks' => '9'], 'where' => "rank = 9"],
        ['label' => 'Challenger', 'attrs' => ['ranks' => '10'], 'where' => "rank = 10"],
        ['label' => 'Top Lane', 'attrs' => ['roles' => 'TopLane'], 'where' => "CONCAT('|', roles, '|') LIKE '%|TopLane|%'"],
        ['label' => 'Jungle', 'attrs' => ['roles' => 'Jungle'], 'where' => "CONCAT('|', roles, '|') LIKE '%|Jungle|%'"],
        ['label' => 'Mid Lane', 'attrs' => ['roles' => 'MidLane'], 'where' => "CONCAT('|', roles, '|') LIKE '%|MidLane|%'"],
        ['label' => 'ADC', 'attrs' => ['roles' => 'AdCarry'], 'where' => "CONCAT('|', roles, '|') LIKE '%|AdCarry|%'"],
        ['label' => 'Support', 'attrs' => ['roles' => 'Support'], 'where' => "CONCAT('|', roles, '|') LIKE '%|Support|%'"],
...
// ---------- Fetch accounts (AJAX) ----------
  ...
  function buildPageBar(page, total) {
    ...
    // Mobile layout: compact single row, previous, current, ellipsis, last, next.
    if (isMobile) {
...
function fetchAccounts({ page=1 } = {}) {
  ...
  // Dropdowns can be portalled to <body> for positioning. In that state their
  // checkboxes are no longer descendants of #shopFilters, so serialize() alone
  // silently drops the selected facets. Rebuild those values from the live DOM.
...
(function linkSellerNames() {
    ...
    // Initial + nach Ajax-Nachladen, debounced to avoid repeated DOM scans on mobile.
    process();
    ...
    if (grid && 'MutationObserver' in window) {
        ...
        new MutationObserver(function(){
            ...
            sellerTimer = setTimeout(process, 80);
...
@media (max-width: 767px){
  ...
  body.ranked-accounts-page header .content h1{
    display:block !important;
    margin:0 0 10px !important;
    font-size:28px !important;
    line-height:1.05 !important;
  }
  body.ranked-accounts-page header .content p{
    display:block !important;
...
<style id="lb-mobile-final-pagination-content-fix">
@media (max-width: 767px){
  body.ranked-accounts-page header{
  ...
  body.ranked-accounts-page .shop-pagination{
    padding:18px 10px 98px !important;
    margin:18px 0 0 !important;
  }
  body.ranked-accounts-page .page-bar{
    width:100% !important;
    gap:8px !important;
    justify-content:center !important;
  }
  body.ranked-accounts-page .page-btn{
  ...
  body.ranked-accounts-page .page-btn.is-active{
    background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%) !important;
    border-color:rgba(139,92,246,.72) !important;
    color:#fff !important;
  }
  body.ranked-accounts-page .page-btn.is-nav{
    min-width:74px !important;
    font-size:13px !important;
  }
  body.ranked-accounts-page .page-btn.is-disabled{
    opacity:.42 !important;
  }
  body.ranked-accounts-page .page-ellipsis{
    color:rgba(255,255,255,.58) !important;
...
<script id="lb-mobile-pagination-readable-labels">
(function(){
  function relabel(){
    if(!window.matchMedia || !window.matchMedia('(max-width: 767px)').matches) return;
    document.querySelectorAll('.ranked-accounts-page .page-btn.is-nav').forEach(function(btn){
      var raw = (btn.textContent || '').toLowerCase();
      if(raw.indexOf('prev') !== -1) btn.innerHTML = '<i class="fa-solid fa-chevron-left"></i><span>Prev</span>';
      if(raw.indexOf('next') !== -1) btn.innerHTML = '<span>Next</span><i class="fa-solid fa-chevron-right"></i>';
    });
  }
  document.addEventListener('DOMContentLoaded', function(){
    relabel();
    var target = document.getElementById('shopPagination');
    if(target && 'MutationObserver' in window){
      var pageRelabelTimer = null;
      new MutationObserver(function(){
        clearTimeout(pageRelabelTimer);
        pageRelabelTimer = setTimeout(relabel, 60);
...
@media (max-width: 767px){
  body.ranked-accounts-page #shopFilterbar.shop-filterbar{
    top:calc(var(--lb-content-top, 0px) + 8px) !important;
  }
...
<!-- LoL mobile final overrides, matched to Valorant layout -->
...
<style id="lb-mobile-pagination-lol-row-final">
@media (max-width: 767px){
  body.ranked-accounts-page #shopPagination.shop-pagination{
    width: 100% !important;
    max-width: 100% !important;
...
<style id="lb-lol-typecards-fullwidth-final">
@media (max-width: 767px){
  body.ranked-accounts-page .account-type-cards{
    display:grid !important;
...
<style id="lb-lol-shop-redesign-2026">
body.ranked-accounts-page{
  background:
