...
<?php
    ...
    // Effective counts: use manual list when available, fall back to count-only fields
    $championsDisplayCount = count($champions) > 0
        ? count($champions)
...
<?php
    if (!function_exists('should_hide_lol_division')) {
    ...
    if (!function_exists('format_lol_rank_display')) {
    function format_lol_rank_display($rank, $lp, $division): string
    {
        ...
        if ($lp !== null && (int)$lp !== 0) return $label . ' ' . (int)$lp . 'LP';
        if (should_hide_lol_division($rank)) return $label;
...
$_gRaw      = strtolower(trim((string)($account['game'] ?? 'lol')));
$_s2s       = [
...
$_gameShort = $_s2s[$_gRaw] ?? $_gRaw;
$_isLol     = ($_gameShort === 'lol');
$_isVal     = ($_gameShort === 'val');
$_isTft     = ($_gameShort === 'tft');
$_isCod     = ($_gameShort === 'cod');
$_isGeneric = !$_isLol && !$_isVal && !$_isTft && !$_isCod;
$_gameObj   = function_exists('util_get_game_by_slug') ? util_get_game_by_slug($_gRaw) : null;
$_gameName  = $_gameObj['name'] ?? ucwords(str_replace('-', ' ', $_gRaw));
$_gameData  = json_decode((string)($account['game_data'] ?? '{}'), true) ?: [];
$_accountSchema = function_exists('util_get_game_account_schema') ? util_get_game_account_schema($_gameShort) : [];
$_useDynamicViewSchema = !empty($_accountSchema) && !$_isLol && !$_isVal && !$_isTft;
$_dynamicViewHeaderFields = $_useDynamicViewSchema && function_exists('util_account_schema_fields') ? util_account_schema_fields($_gameShort, 'show_on_view_header') : [];
$_dynamicViewFields = $_useDynamicViewSchema && function_exists('util_account_schema_fields') ? util_account_schema_fields($_gameShort, 'show_on_view') : [];


if (!function_exists('account_view_clean_scalar')) {
    function account_view_clean_scalar($value): string
    {
...
if (!function_exists('account_view_platform_icon_files')) {
    function account_view_platform_icon_files($platform): array
    {
        $value = strtolower(trim((string)$platform));
        $value = str_replace(['.', '-', '_'], '', $value);

        if ($value === '' || strpos($value, 'all platforms') !== false || $value === 'allplatforms') {
            return ['pc.webp', 'playstation.webp', 'xbox.webp', 'battlenet.webp', 'steam.webp'];
        }
        if (strpos($value, 'playstation') !== false || preg_match('/\bps[345]?\b/', $value)) return ['playstation.webp'];
        if (strpos($value, 'xbox') !== false) return ['xbox.webp'];
        if (strpos($value, 'battle') !== false || strpos($value, 'bnet') !== false) return ['battlenet.webp'];
        if (strpos($value, 'steam') !== false) return ['steam.webp'];
        if (strpos($value, 'pc') !== false || strpos($value, 'gamepass') !== false || strpos($value, 'windows') !== false) return ['pc.webp'];

        return [];
    }
}

if (!function_exists('account_view_platform_icons_html')) {
    function account_view_platform_icons_html($platform): string
    {
        $files = account_view_platform_icon_files($platform);
        if (empty($files)) return '';

        $base = rtrim((string)(defined('ASSET_URL') ? ASSET_URL : '/assets'), '/') . '/website/images/platforms/';
        $html = '<span class="platform-icon-list">';
        foreach ($files as $file) {
            $html .= '<img src="' . htmlspecialchars($base . $file, ENT_QUOTES) . '" alt="" class="platform-icon-img">';
        }
        $html .= '</span>';
        return $html;
    }
...
if (!function_exists('account_view_feature')) {
    function account_view_feature(string $label, $value, string $icon = '', bool $isHtmlValue = false): string
    {
        $display = $isHtmlValue ? trim((string)$value) : account_view_clean_scalar($value);
        if ($display === '') return '';
        $iconHtml = $icon !== '' ? '<i class="text-primary me-2 fs-5 ' . htmlspecialchars($icon, ENT_QUOTES) . '"></i>' : '';
        return '<div class="feature"><h6>' . t($label) . '</h6><span>' . $iconHtml . ($isHtmlValue ? $display : htmlspecialchars($display)) . '</span></div>';
    }
}

// Always resolve the seller total directly from seller_stats.
// Generic account routes do not consistently expose the seller ID as $seller['id'],
// therefore also use seller_id from the seller row or the account row.
$sellerIdForTotalSales = (int)(
    $seller['id']
    ?? $seller['seller_id']
    ?? $account['seller_id']
    ?? 0
...
$sellerTotalSalesFallback = 0;
foreach (['seller_total_sales', 'total_sales', 'total_sold', 'seller_sold', 'sold'] as $salesKey) {
    if (isset($seller[$salesKey]) && is_numeric($seller[$salesKey])) {
        $sellerTotalSalesFallback = max($sellerTotalSalesFallback, (int)$seller[$salesKey]);
    }
    if (isset($account[$salesKey]) && is_numeric($account[$salesKey])) {
        $sellerTotalSalesFallback = max($sellerTotalSalesFallback, (int)$account[$salesKey]);
    }
}

$sellerTotalSoldDisplay = lb_db_seller_total_sales(
    $sellerIdForTotalSales,
    $sellerTotalSalesFallback
);
?>

<?php

if (!function_exists('account_view_roblox_experience_map')) {
    function account_view_roblox_experience_map(): array
    {
        return [
...
if (!function_exists('account_view_roblox_key')) {
    function account_view_roblox_key($value): string
    {
...
if (!function_exists('account_view_roblox_experience_meta')) {
    function account_view_roblox_experience_meta(array $account, array $gameData): array
    {
        foreach (['experience', 'experience_game', 'main_experience', 'roblox_experience', 'roblox_game', 'game_experience'] as $key) {
            $value = $gameData[$key] ?? $account[$key] ?? null;
            if ($value === null || $value === '' || (is_array($value) && empty($value))) continue;
            $normalized = account_view_roblox_key($value);
            $map = account_view_roblox_experience_map();
            if (isset($map[$normalized])) {
                $base = rtrim((string)(defined('ASSET_URL') ? ASSET_URL : '/public/assets'), '/') . '/website/images/roblox-icons/';
                return ['name' => $map[$normalized]['label'], 'icon' => $base . $map[$normalized]['file']];
            }
...
if (!function_exists('account_view_game_icon_url')) {
    function account_view_game_icon_url(string $gameSlug, array $account = []): string {
        foreach (['game_icon','icon','gameIcon'] as $key) {
            if (!empty($account[$key])) return (string)$account[$key];
        }
        $gameSlug = strtolower(trim($gameSlug));
        $gameRow = function_exists('util_get_game_by_slug') ? util_get_game_by_slug($gameSlug) : null;
        $icon = is_array($gameRow) ? trim((string)($gameRow['icon'] ?? '')) : '';
        return $icon !== '' ? $icon : rtrim((string)(defined('ASSET_URL') ? ASSET_URL : '/assets'), '/') . '/website/images/icons/' . $gameSlug . '.png';
    }
}
if (!function_exists('account_view_header_icon_html')) {
    function account_view_header_icon_html(string $gameSlug, string $gameShort, array $account, array $gameData): string {
        if (in_array(strtolower(trim($gameSlug)), ['roblox', 'roblox-accounts', 'roblox-account'], true) || strtolower(trim($gameShort)) === 'roblox') {
            $robloxMeta = function_exists('account_view_roblox_experience_meta') ? account_view_roblox_experience_meta($account, $gameData) : ['icon' => ''];
            if (!empty($robloxMeta['icon'])) {
                return '<div class="rank-icon account-view-title-icon"><img src="' . htmlspecialchars($robloxMeta['icon'], ENT_QUOTES) . '" alt="" loading="lazy"></div>';
            }
        }
        $rankValue = null;
        $preferRankIcon = in_array($gameShort, ['lol','val','tft'], true);
        if ($preferRankIcon) foreach (['rank','current_rank'] as $key) {
            if (isset($account[$key]) && $account[$key] !== '') { $rankValue = (int)$account[$key]; break; }
        }
        if ($rankValue !== null && function_exists('util_get_rank_img')) {
            $rankImg = util_get_rank_img($gameShort, 'mini', $rankValue);
            if (!empty($rankImg)) return '<div class="rank-icon account-view-title-icon"><img src="' . htmlspecialchars($rankImg, ENT_QUOTES) . '" alt="" loading="lazy"></div>';
        }
        if ($rankValue !== null && function_exists('util_rank_img')) {
            $rankImg = util_rank_img($gameShort, 'mini', $rankValue);
            if (!empty($rankImg)) return '<div class="rank-icon account-view-title-icon"><img src="' . htmlspecialchars($rankImg, ENT_QUOTES) . '" alt="" loading="lazy"></div>';
        }
        $gameIcon = account_view_game_icon_url($gameSlug, $account);
        return $gameIcon !== '' ? '<div class="rank-icon account-view-title-icon"><img src="' . htmlspecialchars($gameIcon, ENT_QUOTES) . '" alt="" loading="lazy"></div>' : '';
    }
}
if (!function_exists('account_view_compact_title')) {
    function account_view_compact_title(array $account, array $gameData, string $gameSlug, string $fallbackGameName): string {
        if (in_array(strtolower(trim($gameSlug)), ['roblox', 'roblox-accounts', 'roblox-account'], true)) {
            $robloxMeta = function_exists('account_view_roblox_experience_meta') ? account_view_roblox_experience_meta($account, $gameData) : ['name' => ''];
            if (!empty($robloxMeta['name'])) {
                return $robloxMeta['name'];
            }
        }
        $rawTitle = trim((string)($account['title'] ?? ''));
        if (in_array($gameSlug, ['clash-of-clans','clashofclans','coc'], true)) {
            $townHall = $gameData['town_hall_level'] ?? $gameData['townHallLevel'] ?? $gameData['townhall_level'] ?? $account['town_hall_level'] ?? null;
            $level = $gameData['xp_level'] ?? $gameData['level'] ?? $gameData['xpLevel'] ?? $account['xp_level'] ?? $account['level'] ?? null;
            $parts = [];
            if ($townHall !== null && $townHall !== '' && !(is_array($townHall) && empty($townHall))) $parts[] = 'Town Hall ' . (is_array($townHall) ? implode(', ', array_filter(array_map('strval', $townHall))) : (string)$townHall);
            if ($level !== null && $level !== '' && !(is_array($level) && empty($level))) $parts[] = 'Level ' . (is_array($level) ? implode(', ', array_filter(array_map('strval', $level))) : (string)$level);
            if (!empty($parts)) return implode(' · ', $parts);
        }
        $title = $rawTitle !== '' ? $rawTitle : $fallbackGameName . ' Account';
        $title = preg_replace('/\s+/', ' ', trim((string)$title));
        $maxTitleLength = 73;
        if (function_exists('mb_strlen') && mb_strlen($title, 'UTF-8') > $maxTitleLength) {
            $title = rtrim(mb_substr($title, 0, $maxTitleLength, 'UTF-8')) . '…';
        } elseif (!function_exists('mb_strlen') && strlen($title) > $maxTitleLength) {
            $title = rtrim(substr($title, 0, $maxTitleLength)) . '…';
        }
        return $title !== '' ? $title : $fallbackGameName . ' Account';
    }
}
?>
<?= $this->start('styles') ?>
...
@media (min-width:768px){
  ...
  /* Highlights: sit LOWER (12px gap from the image), badges hug their own text */
  ...
  body.view-account-page .saf-slide .account-card .highlights .badge{
    display:inline-flex!important; width:fit-content!important; flex:0 0 auto!important;
...
<style id="lb-generic-view-header-seller-fix-2026">
body.view-account-page #account-page-title {
    display: flex !important;
    align-items: center !important;
    gap: 12px !important;
}
body.view-account-page #account-page-title .account-view-title-icon,
body.view-account-page #account-page-title .rank-icon.account-view-title-icon {
    width: 46px !important;
...
body.view-account-page #account-page-title .account-view-title-icon img {
    display: block !important;
    width: 100% !important;
    height: 100% !important;
    min-width: 0 !important;
...
body.view-account-page #account-page-title .account-view-title-text {
    display: block !important;
    flex: 1 1 auto !important;
    min-width: 0 !important;
    max-width: 100% !important;
...
body.view-account-page .right > .seller-profile-card .seller-profile-card__left {
    min-width: 0 !important;
    flex: 1 1 auto !important;
}
body.view-account-page .right > .seller-profile-card .seller-profile-card__avatar,
body.view-account-page .right > .seller-profile-card img.seller-profile-card__avatar {
    width: 48px !important;
    height: 48px !important;
    min-width: 48px !important;
    min-height: 48px !important;
...
body.view-account-page .right > .seller-profile-card .seller-profile-card__info {
    min-width: 0 !important;
}
body.view-account-page .right > .seller-profile-card .seller-profile-card__name {
    min-width: 0 !important;
    max-width: 100% !important;
}
body.view-account-page .right > .seller-profile-card .seller-profile-card__stats {
    flex: 0 0 auto !important;
}
@media (max-width: 767px) {
    body.view-account-page #account-page-title .account-view-title-icon,
...
<?php
$_accountViewTitle = account_view_compact_title($account, $_gameData, $_gRaw, $_gameName);
            ...
            <div class="seller-profile-card seller-profile-card--mobile-inject">
                <div class="seller-profile-card__left">
                    <?php if (!empty($seller['icon'])): ?>
                        <img src="<?= htmlspecialchars(preg_replace('#(?<!:)//+#', '/', (string)$seller['icon'])) ?>" alt="<?= htmlspecialchars($seller['username']) ?>" class="seller-profile-card__avatar">
                    <?php else: ?>
                    ...
                    <div class="seller-profile-card__info">
                        ...
                        <a href="/sellers/<?= htmlspecialchars($seller['username']) ?>" class="seller-profile-card__name" style="text-decoration:none;color:inherit;">
                            ...
                            <?php if (seller_detail_is_online($seller)): ?><span class="seller-online-dot" title="Online" aria-label="Online"></span><?php endif; ?>
                            <?php if ($rankLabel !== ''): ?>
                ...
                <div class="card-body">
                    ...
                    <div class="features">
                        ...
                        <div class="feature">
                            <h6><?= t('Rank') ?></h6>
                            <span>
                                <img src="<?= ASSET_URL ?>/core/main/img/val/ranks/mini/<?= (int)($account['rank'] ?? 0) ?>.png" class="img-fluid" style="width: 25px; height: auto;">
                                <?= htmlspecialchars($account['rank_label'] ?? '') ?>
                            </span>
                        </div>
                        <?php elseif ($_useDynamicViewSchema): ?>
                            <?php foreach ($_dynamicViewFields as $_vField): ?>
                                <?php
                                    $_vKey = (string)($_vField['key'] ?? '');
                                    $_vVal = function_exists('util_account_schema_value') ? util_account_schema_value($account, $_gameData, $_vField) : ($_gameData[$_vKey] ?? null);
                                    $_vType = $_vField['type'] ?? 'text';
                                    if ($_vType === 'checkbox' && empty($_vVal)) continue;
                                    if ($_vVal === null || $_vVal === '' || (is_array($_vVal) && empty($_vVal))) continue;
                                    $_vLabel = (string)($_vField['label'] ?? ucwords(str_replace('_',' ', $_vKey)));
                                    $_vIcon = (string)($_vField['icon'] ?? '');
                                    if (($_vField['icon_type'] ?? '') === 'platform') {
                                        $_vHtml = '<span class="platform-text-with-icons">' . (function_exists('util_account_platform_icons_html') ? util_account_platform_icons_html($_vVal, 'account-view-platform-icon') : account_view_platform_icons_html($_vVal)) . '<span>' . htmlspecialchars(function_exists('util_account_format_schema_value') ? util_account_format_schema_value($_vVal) : account_view_clean_scalar($_vVal)) . '</span></span>';
                                        echo account_view_feature($_vLabel, $_vHtml, '', true);
                                    } else {
                                        echo account_view_feature($_vLabel, $_vVal, $_vIcon);
                                    }
                                ?>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <?php if (!empty($account['rank_label'])) echo account_view_feature('Rank', $account['rank_label']); ?>
                            <?php
                                $_genericSkip = ['val_rank','val_peak_rank','val_act','val_platform','main_title','cod_titles','platform'];
                                foreach ($_gameData as $_gdk => $_gdv) {
                                    if (in_array($_gdk, $_genericSkip, true)) continue;
                                    if (is_array($_gdv) || is_scalar($_gdv)) {
                                        echo account_view_feature(ucwords(str_replace('_', ' ', $_gdk)), $_gdv);
                                    }
                                }
                            ?>
                        <?php endif; ?>
                        ...
                        <?php elseif ($_isVal): ?>
                            <?php
                                echo account_view_feature('Server', !empty($account['server']) ? util_format_server($account['server']) : '', 'fas fa-server');
                                echo account_view_feature('Level', $account['level'] ?? '', 'fas fa-arrow-turn-up');
                                echo account_view_feature('Platform', '<span class="platform-text-with-icons">' . account_view_platform_icons_html($_gameData['val_platform'] ?? '') . '<span>' . htmlspecialchars(account_view_clean_scalar($_gameData['val_platform'] ?? '')) . '</span></span>', '', true);
                                echo account_view_feature('Valorant Points', $_gameData['val_points'] ?? '', 'fas fa-coins');
                                echo account_view_feature('Radianite', $_gameData['val_radianite'] ?? '', 'fas fa-gem');
                            ?>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <div class="card-header">
                    <div class="nav-tabs">
                        <a href="#champs-list" class="active">
                            <i class="fas fa-helmet-battle me-1"></i><?= t('Champions') ?><span class="count-badge"><?= $championsDisplayCount ?></span>
                        </a>
                        <a href="#skins-list">
                            <i class="fas fa-masks-theater me-1"></i><?= t('Skins') ?><span class="count-badge"><?= $skinsDisplayCount ?></span>
                        </a>
                        <a href="#roles-list">
                    ...
                    <div class="totals">
                        ...
                        <form action="<?= AJAX_URL ?>" class="ajax-form">
                            ...
                            <button type="button"
                                    ...
                                    onclick="return window.openSellerChatModal ? window.openSellerChatModal(event) : true;" data-seller-chat-open
                                    data-toggle="modal"
...
<?php
// More from seller: only same game and never the current account.
$seller_accounts = is_array($seller_accounts ?? null) ? $seller_accounts : [];
$currentAccountIdForMore = (int)($account['id'] ?? 0);
$currentGameForMore = strtolower(trim((string)($account['game'] ?? '')));
if ($currentGameForMore !== '') {
    $seller_accounts = array_values(array_filter($seller_accounts, function($sa) use ($currentGameForMore, $currentAccountIdForMore) {
        $saGame = strtolower(trim((string)($sa['game'] ?? '')));
        $saId = (int)($sa['id'] ?? 0);
        return $saId !== $currentAccountIdForMore && $saGame === $currentGameForMore;
    }));
}
?>
    ...
    <div class="seller-accounts-fullwidth__inner">
        <div class="seller-accounts-fullwidth__head">
            ...
            <div class="seller-accounts-fullwidth__controls">
                ...
                <?php endif; ?>
                <a href="/<?= htmlspecialchars($_slugForUrl ?? ($account['game'] ?? 'league-of-legends'), ENT_QUOTES) ?>/accounts" class="saf-viewall"><?= t('View all') ?> <i class="fas fa-arrow-right"></i></a>
            </div>
        </div>
        <?php
        ...
        <div class="saf-slider">
            ...
            <div class="saf-slide">
                <?php echo $this->insert('website/components/accounts/account-cards', ['accounts' => [$_sa], 'game' => $currentGameForMore ?: ($account['game'] ?? '')]); ?>
            </div>
            <?php endforeach; ?>
        </div>
    </div>
...
$lol_rank    = function_exists('util_get_lol_rank') ? util_get_lol_rank((int)($account['rank'] ?? 0)) : '';
$item_type   = htmlspecialchars($lol_rank ?: 'LoL Account', ENT_QUOTES, 'UTF-8');
$item_server = htmlspecialchars(strtoupper((string)($account['server'] ?? '')), ENT_QUOTES, 'UTF-8');
$item_price  = $accountPriceDisplay['with_symbol'] ?? '';
$item_img    = '';
$item_fa     = 'fa-solid fa-gamepad';
?>
<?php if ($seller_id): ?>
<style>
/* ════════════════════════════════════
...
<script>
    $(document).ready(function () {
        // Gallery slider: drag/swipe should move the slider only.
        // The lightbox opens only on a real click, not after dragging and releasing the mouse.
        ...
        function markGalleryAsDragged($gallery) {
            galleryPointer.moved = true;
            gallerySuppressClickUntil = Date.now() + 900;
            if ($gallery && $gallery.length) {
                $gallery.addClass('is-dragging');
                window.clearTimeout($gallery.data('dragClearTimer'));
                $gallery.data('dragClearTimer', window.setTimeout(function () {
        ...
        $('.gallery').each(function () {
            ...
            $gallery
                .on('mousedown touchstart pointerdown', function (event) {
                    var point = getGalleryPoint(event);
                    galleryPointer.down = true;
                    galleryPointer.startX = point.clientX || 0;
                    galleryPointer.startY = point.clientY || 0;
                    galleryPointer.moved = false;
                    galleryPointer.gallery = $gallery;
                })
                .on('mousemove touchmove pointermove', function (event) {
                    if (!galleryPointer.down) return;
                    var point = getGalleryPoint(event);
                    var diffX = Math.abs((point.clientX || 0) - galleryPointer.startX);
                    var diffY = Math.abs((point.clientY || 0) - galleryPointer.startY);
                    if (diffX > 6 || diffY > 6) {
                        markGalleryAsDragged($gallery);
                    }
                })
                .on('swipe beforeChange edge', function () {
                    markGalleryAsDragged($gallery);
                });
            ...
            $gallery.slick({
                ...
                touchMove: true,
                touchThreshold: 8,
                prevArrow: $card.find('.prev'),
                nextArrow: $card.find('.next'),
                responsive: [
        ...
        $(document).on('mouseup touchend touchcancel pointerup pointercancel', function () {
            if (galleryPointer.moved && galleryPointer.gallery) {
                markGalleryAsDragged(galleryPointer.gallery);
            }
            galleryPointer.down = false;
            galleryPointer.gallery = null;
        });

        // Capture phase: stop baguetteBox / browser link handling before it can open.
        document.addEventListener('click', function (event) {
            var link = event.target.closest ? event.target.closest('.gallery a') : null;
            if (!link) return;

            var shouldBlock = galleryPointer.moved || Date.now() < gallerySuppressClickUntil || !!event.target.closest('.gallery.is-dragging');
            if (shouldBlock) {
...
<style>
/* Center Buy Now content inside More from seller account cards */
.seller-accounts-fullwidth .saf-slider:not(.slick-initialized) {
    display: grid !important;
...
.seller-accounts-fullwidth .saf-slider:not(.slick-initialized) .saf-slide {
    width: auto !important;
    min-width: 0;
}
...
.seller-accounts-fullwidth .account-card .totals .btn,
.seller-accounts-fullwidth .account-card .totals a.btn,
.seller-accounts-fullwidth .account-card .totals button.btn,
.saf-slide .account-card .totals .btn,
.saf-slide .account-card .totals a.btn,
.saf-slide .account-card .totals button.btn {
    display: inline-flex !important;
    align-items: center !important;
    justify-content: center !important;
    text-align: center !important;
...
.seller-accounts-fullwidth .account-card .totals .btn i,
.seller-accounts-fullwidth .account-card .totals .btn svg,
.saf-slide .account-card .totals .btn i,
.saf-slide .account-card .totals .btn svg {
    margin-left: 0 !important;
    flex-shrink: 0 !important;
}
</style>
