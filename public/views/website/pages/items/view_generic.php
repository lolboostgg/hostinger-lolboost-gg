...
$images = array_values(array_filter(array_map(function ($img) {
    $img = trim((string)$img);
    if ($img === '') return '';
    if (preg_match('#^https?://#i', $img) || strpos($img, '/') === 0) return $img;
    return rtrim((string)(defined('BASE_URL') ? BASE_URL : ''), '/') . '/' . ltrim($img, '/');
}, $images)));
...
if (!function_exists('items_shop_slugify')) {
    function items_shop_slugify($value): string {
        $value = strtolower(trim((string)$value));
        $value = preg_replace('/[^a-z0-9]+/i', '-', $value);
        return trim((string)$value, '-');
    }
}
if (!function_exists('items_shop_game_meta')) {
    function items_shop_game_meta($gameSlug): array {
        $slug = items_shop_slugify($gameSlug ?: 'league-of-legends');
        $meta = [
            'slug' => $slug,
        ...
        if (function_exists('util_get_all_games')) {
            try {
                foreach ((array)util_get_all_games(true) as $gameRow) {
                    $rowSlug = items_shop_slugify($gameRow['slug'] ?? '');
                    if ($rowSlug === '' || $rowSlug !== $slug) continue;
                    $meta['name'] = (string)($gameRow['name'] ?? $meta['name']);
                    if (!empty($gameRow['icon'])) {
                        $meta['icon'] = (string)$gameRow['icon'];
                    }
                    break;
                }
            } catch (Throwable $e) {}
...
$routeGameSlug = strtolower(trim((string)($gameConfig['slug'] ?? ($game ?? ''))));
$itemGameSlugSource = $routeGameSlug;
if ($itemGameSlugSource === '' && !empty($item['game_id']) && function_exists('util_get_all_games')) {
    try {
        foreach ((array)util_get_all_games(true) as $_gameRow) {
            if ((int)($_gameRow['id'] ?? 0) === (int)$item['game_id']) {
                $itemGameSlugSource = (string)($_gameRow['slug'] ?? '');
                if (!empty($_gameRow['name'])) $gameConfig['name'] = $_gameRow['name'];
                if (!empty($_gameRow['icon'])) $gameConfig['icon'] = $_gameRow['icon'];
                break;
            }
        }
    } catch (Throwable $e) {}
}
if ($itemGameSlugSource === '') $itemGameSlugSource = (string)($item['game'] ?? 'league-of-legends');

$itemGameMeta = items_shop_game_meta($itemGameSlugSource);
if (!empty($gameConfig['name'])) $itemGameMeta['name'] = (string)$gameConfig['name'];
if (!empty($gameConfig['icon'])) $itemGameMeta['icon'] = (string)$gameConfig['icon'];
$itemGameSlug = $itemGameMeta['slug'];
$itemGameName = $itemGameMeta['name'];
$itemGameIcon = $itemGameMeta['icon'];
$isLolItem = in_array($itemGameSlug, ['league-of-legends', 'lol'], true);
$itemData = json_decode((string)($item['item_data'] ?? '{}'), true);
if (!is_array($itemData)) $itemData = [];
foreach (['type','server','game'] as $_k) if (!empty($item[$_k]) && empty($itemData[$_k])) $itemData[$_k] = $item[$_k];
if (!function_exists('item_waiting_time_label')) {
    function item_waiting_time_label(array $item, array $itemData = []): string {
        $amount = isset($item['waiting_time_amount']) ? (int)$item['waiting_time_amount'] : (int)($itemData['waiting_time_amount'] ?? 0);
        $unit = strtolower(trim((string)($item['waiting_time_unit'] ?? ($itemData['waiting_time_unit'] ?? ''))));
...
$itemSchema = is_array($itemSchema ?? null) ? $itemSchema : (function_exists('lb_get_game_item_schema') ? lb_get_game_item_schema($itemGameSlug) : []);
$itemSchemaFields = (!empty($itemSchema['fields']) && is_array($itemSchema['fields'])) ? array_values($itemSchema['fields']) : [];
$itemSchemaFieldMap = [];
foreach ($itemSchemaFields as $_schemaField) {
    $_schemaKey = trim((string)($_schemaField['key'] ?? ''));
    if ($_schemaKey === 'item_type') $_schemaKey = 'type';
    if ($_schemaKey !== '') $itemSchemaFieldMap[$_schemaKey] = $_schemaField;
}
$rawItemType = trim((string)($itemData['type'] ?? $itemData['item_type'] ?? ($item['type'] ?? '')));
$showItemTypeField = $rawItemType !== '' && isset($itemSchemaFieldMap['type']);

$typeLabel = $showItemTypeField ? items_shop_type_label($rawItemType) : '';
$typeImg   = ($isLolItem && $showItemTypeField) ? items_shop_type_img($rawItemType) : null;
$typeFa    = $showItemTypeField ? items_shop_type_fa($rawItemType) : 'fa-solid fa-gamepad';
$displayIconImg = ($showItemTypeField && $typeImg) ? $typeImg : $itemGameIcon;
$displayIconFa  = ($showItemTypeField && !$typeImg) ? $typeFa : 'fa-solid fa-gamepad';
$displayIconLabel = $showItemTypeField ? $typeLabel : $itemGameName;
    ...
    <h3 class="title">
        <div class="rank-icon">
            <?php if (!empty($displayIconImg)): ?>
                <img src="<?= htmlspecialchars($displayIconImg) ?>" alt="<?= htmlspecialchars($displayIconLabel) ?>">
            <?php else: ?>
                <i class="<?= htmlspecialchars($displayIconFa) ?>"></i>
            <?php endif; ?>
        </div>
...
<div class="layout">
            ...
            <div class="card-body">
                ...
                <div class="item-feature-grid">
                    <div class="item-feature">
                        <div class="item-feature__label"><?= t('Game') ?></div>
                        <div class="item-feature__value">
                            <?php if (!empty($itemGameIcon)): ?>
                                <img src="<?= htmlspecialchars($itemGameIcon) ?>" alt="<?= htmlspecialchars($itemGameName) ?>">
                            <?php else: ?>
                                <i class="fa-solid fa-gamepad"></i>
                            <?php endif; ?>
                            <?= htmlspecialchars($itemGameName) ?>
                        </div>
                    </div>
                    <div class="item-feature">
                        <div class="item-feature__label"><?= t('Waiting Time') ?></div>
                        <div class="item-feature__value">
                            <i class="fa-solid fa-clock"></i>
                            <?= htmlspecialchars(t('Waiting') . ' ' . item_waiting_time_label((array)$item, (array)$itemData)) ?>
                        </div>
                    </div>
                    <?php if (isset($itemSchemaFieldMap['server']) && !empty($itemData['server'] ?? $item['server'] ?? '')): ?>
                    <div class="item-feature">
                        <div class="item-feature__label"><?= htmlspecialchars((string)($itemSchemaFieldMap['server']['label'] ?? t('Server'))) ?></div>
                        <div class="item-feature__value">
                            <i class="<?= htmlspecialchars((string)($itemSchemaFieldMap['server']['icon'] ?? 'fa-solid fa-earth-europe')) ?>"></i>
                            <?= htmlspecialchars(function_exists('util_format_server_code') ? util_format_server_code($itemData['server'] ?? $item['server'] ?? '') : strtoupper((string)($itemData['server'] ?? $item['server'] ?? ''))) ?>
                        </div>
                    </div>
                    <?php endif; ?>
                    <?php if ($showItemTypeField): ?>
                    <div class="item-feature">
                        <div class="item-feature__label"><?= htmlspecialchars((string)($itemSchemaFieldMap['type']['label'] ?? t('Type'))) ?></div>
                        <div class="item-feature__value">
                            <?php if ($typeImg): ?>
                    ...
                    <?php endif; ?>
                    <?php if (!empty($itemSchemaFields)): ?>
                      <?php foreach ($itemSchemaFields as $_field):
                        $_key = trim((string)($_field['key'] ?? ''));
                        if ($_key === 'item_type') $_key = 'type';
                        if ($_key === '' || in_array($_key, ['type','server','item_type','delivery_time','waiting_time_amount','waiting_time_unit','waiting_time_minutes','waiting_time_label','title','description','price','stock','images','game'], true)) continue;
                        if (empty($_field['show_on_view']) && empty($_field['show_on_view_header']) && empty($_field['card'])) continue;
                        $_val = $itemData[$_key] ?? null;
                        if ($_val === null || $_val === '' || (is_array($_val) && empty($_val))) continue;
                        $_label = (string)($_field['label'] ?? ucwords(str_replace('_', ' ', $_key)));
                        $_display = is_array($_val) ? implode(', ', array_filter(array_map('strval', $_val))) : (string)$_val;
                      ?>
                      <div class="item-feature">
                        <div class="item-feature__label"><?= htmlspecialchars($_label) ?></div>
                        <div class="item-feature__value"><i class="<?= htmlspecialchars((string)($_field['icon'] ?? 'fa-solid fa-circle-check')) ?>"></i><?= htmlspecialchars($_display) ?></div>
                      </div>
                      <?php endforeach; ?>
                    <?php endif; ?>
                </div>
    ...
    <div class="right">
        ...
        <?php if (!empty($seller)):
    $sellerRankMeta = seller_rank_icon_meta($seller['rank'] ?? '', $seller['rank_icon'] ?? '');
?>
        <!-- Seller Profile Card -->
        <div class="seller-profile-card">
            <div class="seller-profile-card__left">
                ...
                <?php else: ?>
                    <div class="seller-profile-card__avatar-placeholder"><i class="fas fa-user"></i></div>
                <?php endif; ?>
                <div class="seller-profile-card__info">
                    ...
                    <a href="/sellers/<?= htmlspecialchars($seller['username']) ?>" class="seller-profile-card__name" style="text-decoration:none;color:inherit;">
                        ...
                        <span class="seller-profile-card__rank" title="<?= htmlspecialchars($sellerRankMeta['label']) ?>">
                            <i class="<?= htmlspecialchars($sellerRankMeta['class']) ?>" style="color:<?= htmlspecialchars($sellerRankMeta['color'], ENT_QUOTES) ?>;"></i>
                        </span>
                    </a>
                </div>
            </div>
            <div class="seller-profile-card__stats">
                ...
                <div class="seller-stat seller-stat--green">
                    <div class="seller-stat__value"><i class="fas fa-thumbs-up" style="font-size:12px;"></i></div>
        ...
        <div class="card item-checkout-card" id="hide-sticky">
            <div class="card-body">
                ...
                <form action="<?= AJAX_URL ?>" class="ajax-form">
                    ...
                    <input type="hidden" name="game_id"  value="<?= (int)($item['game_id'] ?? ($gameConfig['id'] ?? 0)) ?>">
                    <input type="hidden" name="game_slug" value="<?= htmlspecialchars($itemGameSlug) ?>">
...
<div class="seller-accounts-fullwidth <?= $sellerItemsCount === 1 ? 'seller-accounts-fullwidth--single' : '' ?>">
    <div class="seller-accounts-fullwidth__inner">
        <div class="seller-accounts-fullwidth__head">
            ...
            <div class="seller-accounts-fullwidth__controls">
                ...
                <a href="<?= BASE_URL ?>/<?= rawurlencode($itemGameSlug) ?>/items" class="saf-viewall"><?= t('View all') ?> <i class="fas fa-arrow-right"></i></a>
            </div>
            <?php endif; ?>
        </div>
    ...
    <div class="saf-slider">
        <?php foreach ($sellerItemsFiltered as $more):
            ...
            $moreGameSlug = strtolower(trim((string)($more['game_slug'] ?? $more['game'] ?? $itemGameSlug)));
            if (in_array($moreGameSlug, ['lol', 'league'], true)) { $moreGameSlug = 'league-of-legends'; }
            if ($moreGameSlug === '') { $moreGameSlug = $itemGameSlug; }
            $moreItemUrl = BASE_URL . '/' . rawurlencode($moreGameSlug) . '/item/' . rawurlencode($moreSlugOrId);
            $moreTypeLabel = items_shop_type_label((string)($more['type'] ?? ''));
            // League-only artwork; other games use the Font Awesome type icon instead.
            $moreTypeImg   = $isLolItem ? items_shop_type_img((string)($more['type'] ?? '')) : null;
        ...
        <div class="saf-slide">
            <div class="account-card">
                <a class="cover-link" href="<?= htmlspecialchars($moreItemUrl) ?>">
                    <div class="top-row">
                        <div class="info-col">
                            <div class="title">
...
$item_type   = htmlspecialchars($displayIconLabel, ENT_QUOTES, 'UTF-8');
...
$item_img    = $displayIconImg ?? '';
$item_fa     = $displayIconFa ?? 'fa-solid fa-tag';
?>
<?php if ($seller_id): ?>
<style>
/* ════════════════════════════════════
