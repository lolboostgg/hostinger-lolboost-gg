<?= $this->layout('seller/layouts/main', ['meta' => $meta ?? ['title' => 'Account Orders | LoLBoost.gg']]) ?>

<?php $accounts = array_values(array_filter(is_array($orders ?? null) ? $orders : [], static fn($account) => (int)($account['sold'] ?? 0) === 1)); ?>
  ...
  <div class="al-hero">
    <div class="al-hero-left">
      ...
      <div>
        ...
        <p class="al-hero-sub"><?= count($accounts ?? []) ?> sold account<?= count($accounts ?? []) !== 1 ? 's' : '' ?> total</p>
      </div>
    </div>
    ...
    <table class="al-table" id="alGrid">
      ...
      <tbody id="alTbody">
        <?php if (!empty($accounts)): foreach ($accounts as $acc):
          ...
          $accountTitle = '';
          if (!empty($schema) && !empty($schema['title_field'])) {
              foreach (($schema['fields'] ?? []) as $__field) {
        ...
        <tr class="al-row"
          ...
          <td class="al-row-link" onclick="window.location='<?= BASE_URL ?>/seller-area/account-order/<?= (int)$acc['id'] ?>'"><span class="al-col-id">#<?= (int)$acc['id'] ?></span></td>
          <td class="al-row-link" onclick="window.location='<?= BASE_URL ?>/seller-area/account-order/<?= (int)$acc['id'] ?>'">
...
require_once dirname(__DIR__) . '/_seller_rank.php';
          ...
          <td class="text-end" onclick="event.stopPropagation()">
            <div class="al-actions-wrap">
              ...
              <div class="al-actions-menu">
                ...
                <a class="al-action-item" href="<?= BASE_URL ?>/seller-area/account-order/<?= (int)$acc['id'] ?>">
        ...
        <tr><td colspan="10">
          <div class="al-empty">
            ...
            <div style="font-weight:900;font-size:1rem;color:rgba(255,255,255,.6);margin-bottom:6px;">No sold accounts yet</div>
            <div style="font-size:.85rem;">Sold accounts will appear here automatically.</div>
          </div>
        </td></tr>
        <?php endif ?>
      </tbody>
