	...
	init:function() {
		...
		document.querySelectorAll("[data-bs-popover-dark]").forEach((function(t) {
					t.addEventListener("click", (function(t) {
								const e=document.querySelectorAll(".popover"); e.length&&e[e.length-1].classList.add("popover-dark")
							}))
				}))
	}
...
const HSBsDropdown= {
	...
	setAnimations() {
		window.addEventListener("show.bs.dropdown", (t=> {
					const e=t.target.closest(".table-responsive"); e&&(this.scrollEvent=function() {
							new bootstrap.Dropdown(t.target).hide()
						}

						, e.addEventListener("scroll", this.scrollEvent)); if( !t.target.hasAttribute("data-bs-dropdown-animation"))return; const i=t.target.nextElementSibling; i.style.opacity=0, setTimeout((()=> {
		...
		window.addEventListener("hide.bs.dropdown", (t=> {
					const e=t.target.closest(".table-responsive"); e&&e.removeEventListener("scroll", this.scrollEvent); if( !t.target.hasAttribute("data-bs-dropdown-animation"))return; const i=t.target.nextElementSibling; setTimeout((()=> {
	...
	openOnHover() {
		Array.from(document.querySelectorAll("[data-bs-open-on-hover]")).forEach((t=> {
					var e; const i=new bootstrap.Dropdown(t); function o() {
						e=setTimeout((()=> {
									i.hide()
								}), 500)
					}
...
HSBsValidation= {
	init(t, e) {
		...
		return Array.prototype.slice.call(i).forEach((t=> {
					for(const e in validators)Array.prototype.slice.call(t.querySelectorAll(`[${e}]`)).forEach(validators[e]); this.addVlidationListners(t.elements), t.addEventListener("submit", (i=> {
								t.checkValidity()?this.onSubmit({
									event:i, form:t, options:e
...
addVlidationListners(t) {
	Array.prototype.slice.call(t).forEach((t=> {
				const e=t.closest("[data-hs-validation-validate-class]"); e&&(t.addEventListener("input", (t=>this.updateFieldStete(t.target))), t.addEventListener("focus", (t=>e.classList.add("focus"))), t.addEventListener("blur", (t=>e.classList.remove("focus"))))
			}))
}
...
function mergeDeep(t, ...e) {
	...
	if(isObject(t)&&isObject(i))for(const e in i)isObject(i[e])?(t[e]||Object.assign(t, {
			[e]: {}
...
legend(t) {
	if(t.options.hasOwnProperty("matrixLegend")) {
		for(var e=t.data.datasets[0].data[0].v, i=t.data.datasets[0].data[0].v, o=1; o<t.data.datasets[0].data.length; o++)t.data.datasets[0].data[o].v<e&&(e=t.data.datasets[0].data[o].v),
		t.data.datasets[0].data[o].v>i&&(i=t.data.datasets[0].data[o].v);
		e=e.toFixed(),
		i=i.toFixed();
		var n=[],
		l=t.options.matrixLegend.hasOwnProperty("stepSize")?t.options.matrixLegend.stepSize: i/10, s=t.options.matrixBackgroundColord.hasOwnProperty("additionToValue")?t.options.matrixBackgroundColord.additionToValue:5, a=document.querySelector(t.options.matrixLegend.container);
...
tooltip() {
	window.addEventListener("mousemove", (function(t) {
				if( !t.target.closest("canvas")) {
					let t=document.querySelector(".hs-chartjs-tooltip-matrix"); t&&t.parentElement.removeChild(t)
				}
			}))
}
...
extendOptions(t, e, i, o) {
	...
	return o(t)&&o(e)?(Object.keys(e).forEach((l=> {
					const s=t[l], a=e[l]; Array.isArray(s)&&Array.isArray(a)?i?(t[l]=s.map(((t, e)=>a.length<=e?t:n(t, a[e], i))), a.length>s.length&&(t[l]=t[l].concat(a.slice(s.length)))):t[l]=s.concat(a):o(s)&&o(a)?t[l]=n(Object.assign({}
...
l.options.plugins.tooltip.external=function(t) {
	...
	if(e||((e=document.createElement("div")).id="chartjsTooltip", e.style.opacity=0, e.classList.add("hs-chartjs-tooltip-wrap"), e.classList.add("hs-chartjs-tooltip-matrix"), e.innerHTML='<div class="hs-chartjs-tooltip"></div>', document.body.appendChild(e)), "matrix" !==l.type&&0===t.tooltip.opacity)return e.style.opacity=0,
	void e.remove();
...
_setTheme(t, e) {
	...
	return n(t)&&n(e)?(Object.keys(e).forEach((l=> {
					const s=t[l], a=e[l]; Array.isArray(s)&&Array.isArray(a)?o?(t[l]=s.map(((t, e)=>a.length<=e?t:i(t, a[e], o))), a.length>s.length&&(t[l]=t[l].concat(a.slice(s.length)))):t[l]=s.concat(a):n(s)&&n(a)?t[l]=i(Object.assign({}
...
_updateTheme(t) {
	Object.keys(this.themes).forEach((t=> {
				let e=this.collection.findIndex((e=>e.id===t)); e=e<0?t:e; const i=this.collection[e]; if(i) {
					let e=this.extendOptions(i.$el, i.options, i.dataSettings, t); i.$initializedEl.data=e.data, i.$initializedEl.options=e.options, i.$initializedEl.update()
				}
			}))
}
}
