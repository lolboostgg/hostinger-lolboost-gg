...
// Manual RR inputs (keep existing functions used by buttons)
startRRInput = document.getElementById('start_rr_input');
endRRInput = document.getElementById('end_rr_input');

if (startRRInput != null) {
  startRRInput.addEventListener('input', function (event) {
    event.target.value = event.target.value.replace(/[^0-9]/g, '');
    if (event.target.value === '') event.target.value = 0;
    else event.target.value = parseInt(event.target.value, 10);
    setTimeout(checkRRDifference, 500);
  });
}
...
function decrementValue(input) {
  ...
  if (!(parseInt(input.value, 10) <= 0)) {
    input.value = parseInt(input.value, 10) - 25;
...
function incrementValue(input) {
  if (!input) return;
  input.value = (parseInt(input.value, 10) || 0) + 25;
  $(input).trigger('change');
  checkRRDifference();
  load_variables();
}
...
function checkRRDifference() {
  ...
  if (type === 'rank' && startRRInput && endRRInput) {
    ...
    if ((e - s) < 50 && (parseInt(start_tier, 10) === 8)) {
      endRRInput.value = s + 50;
      $(endRRInput).trigger('change');
    }
  }
}
