...
// config.php instantiates \ParagonIE\EasyDB\EasyDB, which only exists once Composer's
// autoloader is loaded. The normal web request loads it via index.php, but this
// standalone endpoint bootstraps config.php directly — so load the autoloader first
// or config.php fatals with "Class EasyDB not found". vendor/ sits at the app root
// (one level above app/core, i.e. two above this file when served from app/tasks).
if (!class_exists('ParagonIE\\EasyDB\\EasyDB')) {
    foreach ([
        $coreDir . '/../../vendor/autoload.php', // <root>/app/core -> <root>/vendor
        $coreDir . '/../vendor/autoload.php',
        __DIR__ . '/vendor/autoload.php',        // web-root copy: <root>/vendor
        __DIR__ . '/../../vendor/autoload.php',   // app/tasks copy: <root>/vendor
    ] as $autoload) {
        if (is_file($autoload)) {
